...
```bash
# 1. Создать или ОБНОВИТЬ AppProject (обязательно после каждого изменения манифеста)
kubectl apply -f argocd/appproject-cluster-addons.yaml

# 2. Проверить, что whitelist применился (должен быть group/kind: '*')
kubectl get appproject cluster-addons -n argocd -o yaml | grep -A6 clusterResourceWhitelist
